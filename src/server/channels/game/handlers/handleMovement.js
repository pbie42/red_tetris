...
const { gamePlayersUpdateEmit, gameSetActiveEmit, lobbyUpdateGamesEmit } = require('../emits');
...
function setNextPiece(io, games, game, player) {
...
function verifyGamePlayerPiece(io, games, game, player, piece) {
  ...
  if (!verifyNotInSolid(location, shape, player.getBoard())) {
    const newDisplayBoard = newDisplayBoardWithPiece(player.getPiece(), player.getBoard());
    player.updateDisplayBoard(newDisplayBoard);
    setNextPiece(io, games, game, player);
    gamePlayersUpdateEmit(io, game);
    return false;
  }
  return true;
...
function movePiece(io, games, gameID, playerID, moveFunc) {
  ...
  if (!verifyGamePlayerPiece(io, games, game, player, piece)) return games;
